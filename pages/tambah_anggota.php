...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt = $mysqli->prepare($sql)) {
        ...
        if ($stmt->execute()) {
            $pesan = "✅ Anggota <strong>$nama_lengkap</strong> berhasil ditambahkan.";
        } else {
            $pesan_error = "❌ Gagal menambahkan anggota. Error: " . $stmt->error;
        }
        $stmt->close();
    } else {
        $pesan_error = "❌ Prepare gagal: " . $mysqli->error;
    }
}
?>
    ...
    <div class="card mb-4">
        <div class="card-body">
            <form method="post" enctype="multipart/form-data">
                ...
                <a href="index.php?hal=daftar_anggota" class="btn btn-secondary">Kembali</a>
            </form>
        </div>
    </div></div>
